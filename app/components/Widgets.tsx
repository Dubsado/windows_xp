import React, { useState } from "react";

interface TaskbarIcon {
  id: string;
  name: string;
  icon: JSX.Element;
}

const Widgets: React.FC = () => {
  const [activeIcons, setActiveIcons] = useState<string[]>([]);

  const taskbarIcons: TaskbarIcon[] = [
    {
      id: "browser",
      name: "Browser",
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z" />
        </svg>
      ),
    },
    {
      id: "files",
      name: "Files",
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path d="M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm0 12H4V8h16v10z" />
        </svg>
      ),
    },
    {
      id: "terminal",
      name: "Terminal",
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zM4 18V6h16v12H4z M12 14h6v2h-6v-2z M7.5 14l3-3-3-3 1.5-1.5L12 11l-4.5 4.5z" />
        </svg>
      ),
    },
  ];

  const toggleIcon = (id: string) => {
    setActiveIcons((prev) =>
      prev.includes(id) ? prev.filter((iconId) => iconId !== id) : [...prev, id]
    );
  };

  return (
    <div className="flex-1 flex justify-center space-x-1">
      {taskbarIcons.map((icon) => (
        <button
          key={icon.id}
          onClick={() => toggleIcon(icon.id)}
          className={`
            p-2 rounded-md transition-colors duration-200
            ${
              activeIcons.includes(icon.id)
                ? "bg-gray-600 text-white"
                : "text-gray-300 hover:bg-gray-700 hover:text-white"
            }
          `}
          title={icon.name}
        >
          {icon.icon}
        </button>
      ))}
    </div>
  );
};

export default Widgets;
